...
class CholeskyOptimizer():
    """ Optimizer with Cholesky constraints"""
    ...
    def step(self):
        for W in self.parameters:
            # edw = W.grad + th.diag(th.diag(th.matmul(W.grad, th.diag(th.diag(W.data)-1))))
            # edw = W.grad
            dw_pos = th.tril(W.grad.data) + th.diag(th.diag(W.data)*th.diag(W.data)*th.diag(W.grad.data))
            Gt = W.data - self.lr*dw_pos
            W.data = functional.clip(functional.Cholesky_ExpG_Ins(W.data, Gt))
...
class MixOptimizer():
    ...
    def __init__(self, parameters, optimizer=torch.optim.ASGD, lr=5e-2, *args, **kwargs):
        ...
        self.cholesky_parameters = [param for param in parameters if param.__class__.__name__ == 'CholeskyParameter']
        ...
        self.cholesky_optim = CholeskyOptimizer(self.cholesky_parameters, self.lr)
