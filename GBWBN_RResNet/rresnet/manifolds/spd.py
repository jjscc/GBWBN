...
from rresnet.manifolds import functionals
def _sym(x):
...
class SPD(Manifold):
    ...
    def exp(self, x, u, t=1):
        if self.metric == "aff_inv":
            ...
            x_new = sqrt_x @ functionals.sym_expm.apply(inv_sqrt_x @ u @ inv_sqrt_x) @ sqrt_x
            return x_new
        elif self.metric == "log_euc":
            # return torch.matrix_exp(self.log(x) + t * u)
            return functionals.sym_expm.apply(self.log(x) + t * u)
